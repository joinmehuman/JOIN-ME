# JOIN ME 静的公開面セキュリティ試験報告

- 試験日：2026年8月25日
- 対象：GitHub Pagesの開発確認ページ、保存なし1分体験、日本危機支援レジストリ、公開停止設定

## 判定

- 現在実装されている静的公開面：PASS（46／46）
- 製品公開Gate：`BLOCKED_NOT_IMPLEMENTED`
- 日本市場：`enabled=false`
- 決済：`payment_enabled=false`

この報告の合格は、自由記述入力、AI、認証、暗号化保存、削除、決済の合格を意味しない。

## 今回の整理

1. 7月版の説明ページを、機能の公開状況が正確に分かる開発確認ページへ置換。
2. 実際には開始しないCTA、未提供の料金表示、稼働中と誤解されるAI説明を削除。
3. 古い汎用アニメーションを、モバイルメニューに必要な最小JavaScriptへ置換。
4. HTMLと一致しないCSS、使用していないCSS、版番号表記を削除。
5. Vercel専用の`vercel.json`を削除。
6. GitHub Pages向けにCSP meta、referrer制御、外部自動読込み禁止を検査。
7. Instagramのプロフィールリンクから直接開ける`/trial/`を別ページとして追加。
8. 選択式の1分体験、水面を触った時だけ出る波紋、蓮、最大3層の生成音を追加。
9. 音は初回操作後だけ開始し、音と動きを常時個別停止可能にした。`prefers-reduced-motion`も初期状態へ反映。
10. 1分体験に自由記述、選択内容の外部送信・分析、永続保存、外部素材、決済がないことを検査。
11. 選択内容は送信しない一方、GitHub Pagesの配信に伴うIPアドレス等の処理可能性を分けて開示。
12. iPhone向けに、利用者操作内でAudioContextを開始・再開し、音量を上げたうえでリミッターを通す処理を追加。
13. 基本の水庭は1音、Plus見本は「音の森」の3層音・光を体験中1回だけに分離。

## GitHub Pages上の制限

`Content-Security-Policy`のmeta設定は静的ページで使用できる範囲に限定される。HSTS、`frame-ancestors`など、HTTPレスポンスヘッダーでのみ十分に制御できる項目は本構成の合格対象に含めない。

## 未実装のため試験不能

- 自由記述の問い・回答入力と削除E2E
- AI入力前・出力後の二重安全判定
- 認証・認可
- 暗号化したIndexedDB保存
- 期間パス、決済、権利付与
- 本番相当環境へのDASTと第三者侵入試験

## 再現

```sh
python3 tests/security_static.py
```

合格条件：終了コード0、全試験PASS、危機支援情報が期限内、秘密情報0、Vercel専用設定0。

実行結果：終了コード0、46試験すべてPASS。
